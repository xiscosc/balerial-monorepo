...
export function getStatusUIInfo(status: OrderStatus, getVariant: boolean = false): IUIInfo {
	let colors = ButtonVariant.ORDER_GENERIC;
	let staticColor = '';
	...
	switch (status) {
		case OrderStatus.PENDING:
			...
			bannerColor = 'blue';
			colors = getVariant ? ButtonVariant.ORDER_PENDING_VARIANT : ButtonVariant.ORDER_PENDING;
			statusIcon = IconType.ORDER_PENDING;
			break;
		case OrderStatus.FINISHED:
			staticColor = 'bg-amber-600';
			bannerColor = 'amber';
			colors = getVariant ? ButtonVariant.ORDER_FINISHED_VARIANT : ButtonVariant.ORDER_FINISHED;
			statusIcon = IconType.ORDER_FINISHED;
			break;
		case OrderStatus.PICKED_UP:
			staticColor = 'bg-green-700';
			bannerColor = 'green';
			colors = getVariant ? ButtonVariant.ORDER_PICKED_UP_VARIANT : ButtonVariant.ORDER_PICKED_UP;
		...
		case OrderStatus.QUOTE:
			...
			bannerColor = 'purple';
			colors = getVariant ? ButtonVariant.ORDER_QUOTE_VARIANT : ButtonVariant.ORDER_QUOTE;
...
export function getStatusUIInfoWithPaymentInfo(
	...
	if ((status === OrderStatus.PICKED_UP && !payed) || (status === OrderStatus.FINISHED && payed)) {
		return {
			...uiInfo,
			colors: getVariant ? ButtonVariant.DELETE_VARIANT : ButtonVariant.DELETE,
